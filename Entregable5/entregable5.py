...
def find_lower_energy_seam(m: MatrixGrayImage) -> List[int]:  # TODO: IMPLEMENTAR
    ...
    def funcionRecursiva(fila: int, columna: int) -> int:

        if fila== rows: return 0
        elif fila == rows-1: return m[fila][columna]
        else:
            if (fila,columna) not in memo:
                memo[fila, columna] = (funcionRecursiva(fila+1, columna)+m[fila][columna], [fila+1, columna])
                if(columna > 0):
                    aux = funcionRecursiva(fila+1, columna-1)+m[fila][columna]
                    if aux < memo[fila, columna][0]:
                        memo[fila, columna] = (aux, [fila+1, columna-1])
                if(columna < cols-1):
                    aux = funcionRecursiva(fila + 1, columna + 1)+m[fila][columna]
                    if aux < memo[fila, columna][0]:
                        memo[fila, columna] = (aux, [fila+1, columna+1])
            return memo[fila, columna][0]
    ...
    for camino in range(cols):
        aux = funcionRecursiva(0, camino)
        if minimo > aux:
            minimo = aux
            minimoPos = camino
